...
/**
 * 默认的MQ消息发送器实现，是具体实现，供平台内部使用，不对用户使用。
 * 用户使用请使用DefaultMQProducer类。
 */
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    /**
     * 初始化事务环境:主要是判断线程池存不存在,如果不存在则默认创建
     */
    ...
    public TransactionSendResult sendMessageInTransaction(final Message msg,
        ...
        try {
            //发送消息(与非事务消息一模一样,可见只是属性"MessageConst.PROPERTY_TRANSACTION_PREPARED"的值为true)
        ...
        try {
            // 根据本地事务的状态localTransactionState，向MQ发送事务消息：提交或者回滚
    ...
    /**
     * 本地事务执行完成后，根据事务状态反馈给MQ,是否要提交事务/回滚事务
     * @param sendResult
    ...
    public void endTransaction(
        ...
        //判断本地事务的状态
        switch (localTransactionState) {
            case COMMIT_MESSAGE://本地事务成功，提交MQ,从而消费者可以正常消费消息
                requestHeader.setCommitOrRollback(MessageSysFlag.TRANSACTION_COMMIT_TYPE);
                break;
            case ROLLBACK_MESSAGE://本地事务执行失败，回滚MQ消息
