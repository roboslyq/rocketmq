 ...
 * 1、存储所有元数据停机时间以实现恢复、数据保护可靠性
 * 2、简单理解CommitLog描述的是整个CommitLog目录
 *    而MappedFileQueue描述的则是CommitLog File数组容器
 *    而MappedFile描述一个CommitLog File。
 *
 */
public class CommitLog {
    ...
    * */
    ...
    /**
     * RocketMQ 的 CommitLog 为了避免并发写入，使用一个 PutMessageLock。
     * PutMessageLock 有 2 个实现版本：PutMessageReentrantLock 和 PutMessageSpinLock。
     *      PutMessageReentrantLock 是基于 java 的同步等待唤醒机制；
     *      PutMessageSpinLock 使用 Java 的 CAS 原语，通过自旋设值实现上锁和解锁。
